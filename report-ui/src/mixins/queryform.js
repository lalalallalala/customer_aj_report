...
export default {
  ...
  methods: {
    ...
    analysisChartsData(params, data) {
      ...
      } else if (chartType == "widget-table") {
        //table返回数据定制化处理
        return this.tableChartFn(params.chartProperties, data)
      } else {
        return data;
      }
    },
    //表格返回值处理
    tableChartFn(chartProperties, data){
      //首次选择，需要识别出已选参数。 keysValue = 有值
      var keysValue = [];
      for(var key in chartProperties){
        if(chartProperties[key]){
          keysValue.push(key);
        }
      }
       //没有选择任何参数，则返回空
      ...
      //从data中取出参数所定义的属性值。 keysValue['name','address']
      const ananysicData = [];
      data.forEach(p => {
        //加工处理。
        const temp = {}
        ...
        ananysicData.push(temp);
      })

      // console.log('data',data);
      console.log('ananysicData',ananysicData);
      return ananysicData;
    },
