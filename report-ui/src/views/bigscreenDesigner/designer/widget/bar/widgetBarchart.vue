...
export default {
  ...
  methods: {
    ...
    setOptionsData() {
      ...
      // optionsData.dataType == 'dynamicData'
      //   ? this.staticDataFn(optionsData.staticData)
      //   : this.dynamicDataFn(optionsData.dynamicData,optionsData.refreshTime,optionsSetup);
      // optionsData.dynamicData  判断是否存在这个，若不存在。说明是第一次加载使用填充默认数据呈现。
      optionsData.dynamicData == undefined || optionsData.dynamicData == ''
        ? this.staticDataFn(optionsData.staticData)
        : this.dynamicDataFn(optionsData.dynamicData,optionsData.refreshTime,optionsSetup);
      // console.log(optionsData.dynamicData == undefined || optionsData.dynamicData == '');
    },
    // 静态数据
    staticDataFn(val) {
      const optionsSetup = this.optionsSetup;
