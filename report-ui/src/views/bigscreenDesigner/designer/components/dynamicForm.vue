      ...
      <template v-for="(item, index) in options">
        <div v-if="isShowForm(item, '[object Object]')" :key="index">
          ...
          <dynamicAddTable
            ...
            @change="changed($event, item.name)"
          />
             <!-- 图片专用设置  custom-upload 背景设置也用这个东西 -->
          <dynamicAddImg
            v-if="item.type == 'custom-upload' && inputShow[item.name]"
            v-model="formData[item.name]"
            :chart-type="item.chartType"
            @change="changed($event, 'imageAdress')"
          />
        </div>
...
import dynamicAddImg from "./dynamicAddImg.vue"; //自定义的
...
export default {
  ...
  methods: {
    ...
    changed(val, key) {
      // console.log("无论哪个输入框改变 都需要触发事件 将值回传",val,key);
      if (val.extend) {
    ...
    setDefaultValue() {
      ...
      // console.log(this.formData.layerName);
    },
    // 是否显示 那种格式
    isShowForm(val, type) {
