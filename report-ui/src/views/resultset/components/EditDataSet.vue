        ...
        <el-row :gutter="10">
          ...
          >
            <el-form-item label="数据表" prop="sourceTable">
              <el-select
                v-model.trim="formData.sourceTable"
                class="organisation"
                size="mini"
...
import {createCoding} from "@/api/reportDataSource";
...
export default {
  ...
  methods: {
    ...
    async addOrEditDataSet(row, type) {
      ...
      if (row.id) {
        this.sourceTableDisabled = false;
        ...
        dataSetPreview(row).then(data => {
          ...
          console.log("this.formData.sourceTable",this.formData.sourceCode)
          this.changeSource2(this.formData.sourceCode)
        });
      } else {
    ...
    async handleClickTabs(tab, event) {
      ...
      if (tab.paneName == "third") {
        this.tableDataSql = [],
        this.fieldList = [];
        // if (this.formData.dynSentence.indexOf("delete") > -1 || this.formData.dynSentence.indexOf("DELETE") > -1 ||
        //   this.formData.dynSentence.indexOf("update") > -1 || this.formData.dynSentence.indexOf("UPDATE") > -1 ||
        //   this.formData.dynSentence.indexOf("insert") > -1 || this.formData.dynSentence.indexOf("INSERT") > -1 ||
        //   this.formData.dynSentence.indexOf("query") > -1 || this.formData.dynSentence.indexOf("QUERY") > -1 ||
        //   this.formData.dynSentence.indexOf("add") > -1 || this.formData.dynSentence.indexOf("ADD") > -1 ||
        //   this.formData.dynSentence.indexOf("edit") > -1 || this.formData.dynSentence.indexOf("EDIT") > -1 ) {
        //   this.$message.warning("只允许查询操作");
        //   return;
        // }
        if(this.formData.dynSentence != ''&& this.formData.sourceCode != ''){
          const params = {
          ...
          this.tableDataSql = data.data;
          this.fieldList = data.setList;

          console.log("this.fieldList",this.fieldList)
          console.log("this.tableDataSql",this.tableDataSql)
        }else {
          this.$message.error("请输入查询sql")
        }
      }
    },
    // 必选
...
public class DemoGroovyHandler implements IGroovyHandler {
      ...
      this.$refs[formName].validate(async (valid, obj) => {
        if (valid) {
          if (this.testMassageCode == 200) {
            ...
            this.formData.caseResultFields = JSON.stringify(this.fieldList);
    ...
    changeSourceTable(params){
      this.changeNum = 0;
      this.formData.dynSentence = "select * from " + params;;
    },
    monacoEditorChange(){
      console.log("触发this.changeNum",this.changeNum)
    ...
    // 生成随机编码
    async createEditSetCodingMethods() {
      console.log("调用随机编码")
      const {code, data} = await createCoding();
      if(code != '200') return;
      this.formData.setCode = data;
      console.log("调用随机编码",this.formData.setCode)
    },
  }
};
</script>
