    ...
    >
      ...
      <el-row :gutter="10">
        <el-col
          ...
          :xl="24"
        >
          <template v-if="data.labelValue == '密码'">
            <el-form-item :label="data.labelValue" >
              <el-input v-model.trim="data.value" show-password/>
            </el-form-item>
          </template>
...
export default {
  ...
  methods: {
    async setDataSource() {
      this.dialogForm.sourceName = "";
      this.dialogForm.sourceType = "";
      this.dialogForm.sourceDesc = "";
      this.dialogForm.sourceConfig = "";
    ...
    // 生成随机编码
    async createCodingMethods() {
      const {code, data} = await createCoding();
      if(code != '200') return;
      this.dialogForm.sourceCode = data;
    },
    // 关闭模态框
    closeDialog() {
