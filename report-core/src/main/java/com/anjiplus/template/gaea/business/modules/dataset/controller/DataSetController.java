...
public class DataSetController extends GaeaBaseController<DataSetParam, DataSet, DataSetDto> {
    ...
    /**
     * 根据sourceCode获取所有的数据表
     * @param sourceCode
     * @return
     */
    @GetMapping("/queryAllDataTable/{sourceCode}")
    public ResponseBean queryAllDataTable(@PathVariable("sourceCode")String sourceCode){
        return responseSuccessWithData(dataSetService.queryAllDataTable(sourceCode));
    }
}
